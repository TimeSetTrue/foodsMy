...
function forms(formSelector, modalTimerId) {
  // Forms
  var formsElems = document.querySelectorAll(formSelector);
  var message = {
    loading: 'img/form/spinner.svg',
    success: 'Спасибо! Скоро мы с вами свяжемся',
    failure: 'Что - то пошло не так...'
  };
  formsElems.forEach(function (item) {
    bindPostData(item);
  });
...
function modal(triggerSelector, modalSelect, modalTimerId) {
  ...
  modalTrigger.forEach(function (btn) {
    btn.addEventListener('click', function () {
      return openModal(modalSelect, modalTimerId);
    });
  });
...
function slider(_ref) {
  var nextSlide = _ref.nextSlide,
      ...
      slider = _ref.slider,
      slideElem = _ref.slideElem,
      currentCounters = _ref.currentCounters,
      totalCounters = _ref.totalCounters,
      wrapper = _ref.wrapper,
      slideInner = _ref.slideInner;
  //Slider
  var offerSliderNext = document.querySelector(nextSlide),
  ...
  if (offerSlideElems.length < 10) {
    totalElem.textContent = "0".concat(offerSlideElems.length);
    currentElem.textContent = "0".concat(slideIndex);
  } else {
    totalElem.textContent = offerSlideElems.length;
    currentElem.textContent = slideIndex;
  }
  ...
  offerSlideElems.forEach(function (slide) {
  ...
  for (var i = 0; i < offerSlideElems.length; i++) {
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_8__.default)('.timer', '2021-04-18');
});
})();
